...
Comprehensive performance optimization guide for React applications, designed for AI agents and LLMs. Contains 50+ rules across 7 categories, prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns). Each rule includes detailed explanations, real-world examples comparing incorrect vs. correct implementations, and specific impact metrics to guide automated refactoring and code generation.
...
2. [Bundle Size Optimization](#2-bundle-size-optimization) — **CRITICAL**
   - 2.1 [Prefer Statically Analyzable Paths](#21-prefer-statically-analyzable-paths)
...
3. [Client-Side Data Fetching](#3-client-side-data-fetching) — **MEDIUM-HIGH**
   ...
   - 3.4 [Use SWR for Automatic Deduplication](#34-use-swr-for-automatic-deduplication)
4. [Re-render Optimization](#4-re-render-optimization) — **MEDIUM**
   ...
   - 4.3 [Calculate Derived State During Rendering](#43-calculate-derived-state-during-rendering)
   ...
   - 4.7 [Extract Default Non-primitive Parameter Value from Memoized Component to Constant](#47-extract-default-non-primitive-parameter-value-from-memoized-component-to-constant)
   - 4.8 [Extract to Memoized Components](#48-extract-to-memoized-components)
   - 4.9 [Put Interaction Logic in Event Handlers](#49-put-interaction-logic-in-event-handlers)
   - 4.10 [Don't Define Components Inside Components](#410-dont-define-components-inside-components)
   - 4.11 [Do not wrap a simple expression with a primitive result type in useMemo](#411-do-not-wrap-a-simple-expression-with-a-primitive-result-type-in-usememo)
   - 4.12 [Split Combined Hook Computations](#412-split-combined-hook-computations)
...
5. [Rendering Performance](#5-rendering-performance) — **MEDIUM**
   - 5.1 [Use Activity Component for Show/Hide](#51-use-activity-component-for-showhide)
   - 5.2 [Animate SVG Wrapper Instead of SVG Element](#52-animate-svg-wrapper-instead-of-svg-element)
   - 5.3 [Use Explicit Conditional Rendering](#53-use-explicit-conditional-rendering)
   - 5.4 [CSS content-visibility for Long Lists](#54-css-content-visibility-for-long-lists)
   - 5.5 [Hoist Static JSX Elements](#55-hoist-static-jsx-elements)
   - 5.6 [Prevent Hydration Mismatch Without Flickering](#56-prevent-hydration-mismatch-without-flickering)
   - 5.7 [Suppress Expected Hydration Mismatches](#57-suppress-expected-hydration-mismatches)
   - 5.8 [Use React DOM Resource Hints](#58-use-react-dom-resource-hints)
   - 5.9 [Use defer or async on Script Tags](#59-use-defer-or-async-on-script-tags)
   - 5.10 [Optimize SVG Precision](#510-optimize-svg-precision)
...
6. [JavaScript Performance](#6-javascript-performance) — **LOW-MEDIUM**
   ...
   - 6.2 [Cache Repeated Function Calls](#62-cache-repeated-function-calls)
   ...
   - 6.6 [Early Return from Functions](#66-early-return-from-functions)
   - 6.7 [Use flatMap to Map and Filter in One Pass](#67-use-flatmap-to-map-and-filter-in-one-pass)
   - 6.8 [Hoist RegExp Creation](#68-hoist-regexp-creation)
   - 6.9 [Build Index Maps for Repeated Lookups](#69-build-index-maps-for-repeated-lookups)
   ...
   - 6.12 [Defer Non-Critical Work with requestIdleCallback](#612-defer-non-critical-work-with-requestidlecallback)
...
7. [Advanced Patterns](#7-advanced-patterns) — **LOW**
   ...
   - 7.3 [Initialize App Once, Not Per Mount](#73-initialize-app-once-not-per-mount)
...
### 2.1 Prefer Statically Analyzable Paths
...
Build tools also trace file-system usage statically: `path.join(process.cwd(), someVar)` can widen the set of files the tool has to include, since it can't see which values `someVar` may take.
...
If a library doesn't expose stable deep import paths, configure your bundler to optimize the barrel away automatically (e.g. Vite's `optimizeDeps.include`, webpack's `sideEffects` + module concatenation) instead of importing the whole barrel.

> **TypeScript warning:** Some libraries (notably `lucide-react`) don't ship `.d.ts` files for their deep import paths. Importing from `lucide-react/dist/esm/icons/check` may resolve to an implicit `any` type under `strict` or `noImplicitAny`. Verify the library exports types for its subpaths before relying on direct imports.
...
### 4.10 Don't Define Components Inside Components
...
### 4.11 Do not wrap a simple expression with a primitive result type in useMemo
...
### 4.12 Split Combined Hook Computations
...
In any SSR setup, some values are intentionally different on server vs client (random IDs, dates, locale/timezone formatting). For these *expected* mismatches, wrap the dynamic text in an element with `suppressHydrationWarning` to prevent noisy warnings. Do not use this to hide real bugs. Don’t overuse it.
...
### 5.9 Use defer or async on Script Tags
...
### 7.3 Initialize App Once, Not Per Mount
