...
let
  ...
  nordvpn =
    let
      cli = nordvpn-cli.overrideAttrs (old: {
        preBuild =
          let
    ...
    in
    pkgs.symlinkJoin {
      inherit (nordvpn-cli) pname version meta;
      paths = [
        cli
        nordvpn-gui
      ];
...
in
{
  options.services.nordvpn = {
    enable = lib.mkEnableOption ''
      ...
      See README.md for complete documentation and configuration examples.
      Available at: https://github.com/NordSecurity/nordvpn-linux
    '';
    user = lib.mkOption {
      type = lib.types.str;
      default = defaultUser;
